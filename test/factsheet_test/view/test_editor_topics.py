...
class TestEditorTopics:
    """Unit tests for :class:`.EditorTopics`."""
    ...
    def test_init(self):
        ...
        assert isinstance(target._outline_topics, VTOPICS.DisplayTopicsId)
        assert target._outline_topics.ui_view.get_model() is (
            target._control_sheet.new_view_topics._ui_model)
        actions = target._ui_view.get_action_group(PREFIX_ACTIONS_TOPICS)
        assert isinstance(actions, Gio.SimpleActionGroup)
        assert actions.lookup_action('clear-topics') is not None
        assert actions.lookup_action('collapse-outline') is not None
    ...
    def test_add_actions_to_group(self, NAME_ACTION):
        """Confirm topic outline editor actions added to group.
    ...
    def test_init_signal_change(self):
        ...
        target = VTOPICS.EditorTopics(p_control_sheet=control_sheet)
        # Test
        attribute = getattr(target._outline_topics, NAME_ATTRIBUTE)
        n_handlers = 0
        while True:
    ...
    def test_on_changed_selection_in(self):
        ...
        ui_model_topics = target._outline_topics.ui_view.get_model()
        ...
        target._outline_topics.ui_view.expand_all()
        # Test
        target._outline_topics.ui_selection.select_iter(line_topic)
    ...
    def test_on_changed_selection_none(self):
        ...
        target._outline_topics.ui_selection.select_iter(line_topic)
        # Test
        target._outline_topics.ui_selection.unselect_all()
        name_visible = target._views_topics.ui_view.get_visible_child_name()
    ...
    def test_on_changed_selection_warn(self, monkeypatch, caplog):
        ...
        # Test
        target._outline_topics.ui_selection.select_iter(line_trigger)
        name_visible = target._views_topics.ui_view.get_visible_child_name()
        assert target._name_view_default == name_visible
    ...
    def test_on_delete_topic(self):
        ...
        N_DEPTH = 5
        _ = fill_topics(control_sheet, N_WIDTH, N_DEPTH)
        model, _ = target._outline_topics.ui_selection.get_selected()
        PATH_DELETE = '3:0:0'
        line_delete = model.get_iter_from_string(PATH_DELETE)
        target._outline_topics.ui_selection.get_tree_view().expand_all()
        target._outline_topics.ui_selection.select_iter(line_delete)
        ...
        _model, line = target._outline_topics.ui_selection.get_selected()
        assert line is None
...
class TestDisplayTopicsId:
    """Unit tests for :class:`.DisplayTopicsId`."""
    ...
    def test_init(self):
        ...
        control_sheet = CSHEET.ControlSheet(p_path=None)
        # Test
        target = VTOPICS.DisplayTopicsId(p_control_sheet=control_sheet)
        assert target._control_sheet is control_sheet
        assert target._ui_view.get_model() is (
            target._control_sheet.new_view_topics._ui_model)
        assert (target._ui_selection is target._ui_view.get_selection())
    ...
    def test_add_actions_to_group(self, NAME_ACTION):
        """Confirm topic outline display actions added to group.
    ...
    def test_on_change_depth_warn(self, caplog):
        ...
        log_message = ('Unexpected action: ' + NAME +
                       ' (DisplayTopicsId.on_change_depth)')
        # Test
        target.on_change_depth(action, None)
        assert N_LOGS == len(caplog.records)
        record = caplog.records[LAST]
    ...
    def test_on_switch_columns(
            self, PRE_NAME, PRE_TITLE, POST_NAME, POST_TITLE):
...
class TestModule:
    ...
    @pytest.mark.parametrize('TYPE_TARGET, TYPE_EXPECT', [
        (VTOPICS.UiActionMap, Gio.ActionMap),
        (VTOPICS.UiDisplayTopicsId, Gtk.TreeView),
        (VTOPICS.UiEditorTopics, Gtk.Frame),
        (VTOPICS.UiTopicSelection, Gtk.TreeSelection),
        ])
